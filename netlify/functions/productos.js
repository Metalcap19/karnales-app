...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (method === 'GET') {
      ...
      if (params.id) {
        const rows = await readSheet('Productos!A:K');
    ...
    if (method === 'POST') {
      ...
      const { nombre, rubro, cantidad, precioCompra, precioVenta, activo, imagen, talle } = data;
    ...
    if (method === 'PUT') {
      ...
      const data = JSON.parse(event.body || '{}');
      const { id, nombre, rubro, cantidad, precioCompra, precioVenta, activo, imagen, talle } = data;
      ...
      const actualizado = {
        ...
        talle:             talle     !== undefined ? talle              : (current[10] || ''),
      };
